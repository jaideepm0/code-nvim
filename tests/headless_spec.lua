...
test('Shift+Home and Shift+End select the complete line range', function()
  local env = setup_buffer({ 'aé🙂b' }, { cursor = { 1, 1 } })

  env.actions.select_to_line_boundary('end')

  local selection = env.multi_cursor.primary().selection
  assert_eq({ selection.anchor.col, selection.active.col }, { 1, 8 })
  assert_eq(
    vim.api.nvim_buf_get_text(env.bufnr, 0, selection.anchor.col, 0, selection.active.col, {}),
    { 'é🙂b' },
  ...
  selection = home_env.multi_cursor.primary().selection
  assert_eq({ selection.anchor.col, selection.active.col }, { 1, 0 })
  assert_eq(
    vim.api.nvim_buf_get_text(home_env.bufnr, 0, selection.active.col, 0, selection.anchor.col, {}),
    { 'a' },
    'Shift+Home should select back to the beginning'
  )
end)

test('EOL selection survives the Insert-mode caret being reported on the last byte', function()
  local env = setup_buffer({ 'hello' }, { cursor = { 1, 1 } })

  env.actions.select_to_line_boundary('end')
  vim.api.nvim_win_set_cursor(0, { 1, 4 })
  env.multi_cursor.sync_cursors()

  local cursor = env.multi_cursor.primary()
